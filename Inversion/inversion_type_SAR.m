function s=inversion_type_SAR(d,A,n_patches,n_comp,inversion_opt,kkramp)
...
%lsqnonneg_flag=1;
if positivity_flag==1 || kkramp~=0 || ~isempty(lsqlin_options)
    if fnnls_flag==1 && lsqnonneg_flag==0,
        disp('check positivity...');
        if exist('lsqlin.m'),
            disp('lsqlin used');
            options=optimset('Display','iter','LargeScale','on','DiffMaxChange',1e-1,'DiffMinChange',1e-9,'TolCon',1e-9,'TolFun',1e-9,'TolPCG',1e-9,'TolX',1e-9,'MaxIter',1e9,'MaxPCGIter',1e9);
            s=lsqlin(A,fact*d,[],[],[],[],lb,ub,s_start,options);
        end
    end
    if lsqnonneg_flag==1,
        disp('lsqnonneg used');
        disp('WARNING: lsqnonneg assumes all components will invert for positive slip.')
        disp('Please change to lsqlin for a multicomponent model with positivity');
        XtX=A'*A;Xty=A'*d;
