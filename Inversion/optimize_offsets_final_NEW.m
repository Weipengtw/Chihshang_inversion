function [X_dat,final_offsets] = optimize_offsets_final_combined_NEW2(X_dat,X_err,G_projected_dense,L,S,V,X_time_index)
...
%   $Revision by H. Perfettini: 2.0.0.0 $  $Date: 2014/01/18  $
% load G_combined.mat;
...
X_model = cell(1,n_datasets);%% CORRECTED on 18/01/2014
...
for ii = 1:n_datasets
    ...
    X_dat_curr_dataset=X_dat{ii};
    n_tseries_curr_dataset=size(X_dat_curr_dataset,1);
    n_end=n_start+n_tseries_curr_dataset-1;
    X_model{ii}=X_model_tmp(n_start:n_end,:);
    n_start=n_end+1;
