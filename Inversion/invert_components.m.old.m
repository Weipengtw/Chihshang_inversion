function [L,L_var]=invert_components(U,G_projected_dense,gamma,Lap,n_comp,options)
...
fnnls_flag=1;lsqnonneg_flag=0;direction_factor=1;rake=[];lsqlin_options_flag=0;linprog_options_flag=0;
...
% % Covariance matrix (not implemented)
% U_var_inv = U_var^(-1);
...
if lsqlin_options_flag,
    ...
    [iflag_lsqlin,ind_lsqlin]=find_string_in_cell(options,'lsqlin');
    inversion_opt{n_inversion_opt+1}=options{ind_lsqlin+1};
end
%add linprog options
if linprog_options_flag,
    n_inversion_opt=numel(inversion_opt);
    [iflag_linprog,ind_linprog]=find_string_in_cell(options,'linprog');
    inversion_opt{n_inversion_opt+1}=options{ind_linprog+1};
end

save bef_inv_tmp.mat;
