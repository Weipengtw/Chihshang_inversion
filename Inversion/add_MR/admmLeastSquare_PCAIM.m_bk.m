...
% Modified: Feb. 2015
%%
function m1 = admmLeastSquare_PCAIM(G,Cd,Cm,m0,d,rho,n_iter)
...
for c = 1:n_iter
    % step (1) 
% (1)  find m_(1+k) E argmin f(M,F_k)
      %m_k = inv (Gt * Cd_inv * G + Cm_inv + rho/2 * eye(length(m0)))...
          %* (Gt * Cd_inv * d + Cm_inv * m0' + rho/2 * Gt * (F_k+lambda_k))    
    m_k = XY + rho/2 * XGt * (F_k + lambda_k);
    % test
    %m_k = Cm * Gt *inv(Cd + G * Cm*Gt + inv(Gt*Cd_inv)*rho/2*Cm*Gt);
    %m_k = X*(Y + rho/2 * XGt * (F_k + lambda_k));
    % step (2)
    F_k = max(0,m_k-lambda_k);
