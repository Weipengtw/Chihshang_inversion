function [L,L_var,R_diag,R_rest,R,ramp, GF_ramp]= invert_components_tarantola_RE(U, G_projected_dense, fault_position, all_position, n_comp, options, data_type)
...
%   Written by Mathilde Radiguet, last update 2020/10/20
%   Update 2020/10/20: - includes repeating eathquake as data
%                      - lambda_0 is a vector
%                      - inversion of a vector for GPS data 
...
% (for GPS data) => vector
% U_GPS = [G 1 1 1] [L a b c]
% Ramp parameters are included at the end od the model parameter vectors

% direct problem with different datasets
%  d     =      G                m

% |d_SAR|    |G_SAR  G_ramp      0  |    |m_ss    |
% |d_GPS|  = |G_GPS    0    G_vector| *  |m_ds    |
% |d_RE |    |G_RE     0         0  |    |m_ramp  |
%                                        |m_vector|

% Include reapeating earthquakes
% Include RE as data
% only possible if the inversion rake is fixed.
...
if invert_ramp
    %[G_ramp, m0,n_ramp_dataset,GF_ramp] = set_input4ramp(G_proj, all_position, n_patches, data_type);
...
if fixed_rake_flag == 1 % only half of the matrix is non-zero
    ...
    if two_faults_flag == 1 % two independant faults
        lambda_tecto_1 = lambda_tecto;
...
if invert_ramp
    ...
    n_ramp = 3*n_ramp_dataset; % number of model parameter for ramp
...
if RES_data
    if n_comp ~=1 || fixed_rake_flag ~=1
        error('Error: Repeating Earthquake as data can only be used for Fixed Rake and n_comp=1')
    end
end
...
switch which_space
    case 'model_space' % invert in the model space
        if n_comp ~=1
            error('Error: Check implementation migth not work to invert several components')
        end
        % need to reduce matrix size to avoid singularity with
        % empty matrices (if fixed rake only the first quarter of the
        % Cm matrix is non-zero)
        Cm_mod = blkdiag(Cm(1:n_patches,1:n_patches), Cm_ramp);
        m0_mod = zeros(1,n_patches+n_ramp);
        if RES_data
            G_proj_mod=[G_proj(:,1:n_patches) G_proj(:,end-n_ramp+1:end); G_RE zeros(size(G_RE,1),n_ramp)]; % add RE Green's function
        else
        ...
        if positivity_flag == 1
            ...
            %% add by P.W ,need to reduce matrix size to avoid singularity.
    ...
    case 'data_space' % invert in the data space (no need to compute Cm_inv)
        if invert_ramp || RES_data
%            error('Ramp and repeating earthquakes not (yet) implemented for the data_space');
